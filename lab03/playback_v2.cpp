...
void segmentFrame(cv::Mat &matFrame, double aProbFloorHS[H_level][32][16], double aProbNonFloorHS[64][32][16])
{ 
  cv:: Mat matFrameHSV;
   ...
   for(int iRow = 0; iRow < matFrame.rows;iRow++)
  {
      ...
      {   
          ...
          double probHSVgivenFloor = aProbFloorHS[HSV[0]>>shift_h][HSV[1]>>shift_s][HSV[1]>>shift_v];
          double probHSVgivenNonFloor = aProbNonFloorHS[HSV[0]>>shift_h][HSV[1]>>shift_s][HSV[1]>>shift_v];
...
void getHistrograms(double aProbFloorHS[64][32][16], double aProbNonFloorHS[64][32][16])
{
  ...
  int totalPixels = cFloor + cNonFloor;
    for(int iH =0; iH < H_level; iH++)
   {
         for(int iS =0; iS < S_level; iS++)
         {
            for(int iV=0; iV < V_level; iV++)
            {
                aProbFloorHS[iH][iS][iV] /= cFloor;
                aProbFloorHS[iH][iS][iV] *= (cFloor/totalPixels);

                aProbNonFloorHS[iH][iS][iV] /= cNonFloor;
                aProbNonFloorHS[iH][iS][iV] *= (cNonFloor/totalPixels);
...
void Test(double aProbFloorHS[64][32][16], double aProbNonFloorHS[64][32][16])
{       
       std::vector<std::string> aImages,aMasks;

      // test set image 21 to 30 
       for(int iNimg =0; iNimg< TEST_IMGS_SIZE; iNimg++)
       {
          int digit = (18-(iNimg+10))/10;
          std::string s = "0";
          string s1 ="../video-frames/frame-"+(s *digit) + std::to_string(iNimg+21) + ".png";
          string s2 = "../video-frames/frame-"+ (s *digit) + std::to_string(iNimg+21) + "-mask.png";
...
int main()
{
   ...
   //waitKey(0);
    double aProbFloorHS[64][32][16] = {0};
    double aProbNonFloorHs[64][32][16]= {0};
